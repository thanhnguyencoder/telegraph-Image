...
const LoginButton = ({ onClick, href, children }) => (
  <button
    onClick={onClick}
    className="px-4 py-2 mx-2 w-28 sm:w-28 md:w-20 lg:w-16 xl:w-16 2xl:w-20 bg-blue-500 text-white rounded hidden"
  >
    {children}
  </button>
);
  ...
  return (
    <main className=" overflow-auto h-full flex w-full min-h-screen flex-col items-center justify-between">
      <header className="fixed top-0 h-[50px] left-0 w-full border-b bg-white flex z-50 justify-center items-center">
        <nav className="flex justify-between items-center w-full max-w-4xl px-4">Ông Sói</nav>
        {renderButton()}
      </header>
        ...
        <div className="flex flex-row">
          <div className="flex flex-col">
            ...
            <div className="mb-4 text-sm text-gray-500">
              <p>Kích thước mỗi tệp tải lên tối đa 50 MB (tối đa 20 file trong 1 phút)</p>
              
              <p>Đã lưu trữ tổng cộng <span className="text-cyan-600">{Total}</span> hình ảnh</p>
              
              <p>Các định dạng hỗ trợ image/*, video/*, .zip, .tar, .gz, .rar, .7z, .xz, .bz2</p>
              
              <p>IP của bạn：<span className="text-cyan-600">{IP}</span></p>
            </div>
          </div>
          <div className="flex  flex-col sm:flex-col   md:w-auto lg:flex-row xl:flex-row  2xl:flex-row  mx-auto items-center hidden ">
            <span className=" text-lg sm:text-sm   md:text-sm lg:text-xl xl:text-xl  2xl:text-xl">Tải lên：</span>
            <select
